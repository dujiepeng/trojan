#!/bin/bash
# Author: dujiepeng
# github: https://github.com/dujiepeng/trojan
...
download_url="https://github.com/dujiepeng/trojan/releases/download/"

version_check="https://api.github.com/repos/dujiepeng/trojan/releases/latest"

service_url="https://raw.githubusercontent.com/dujiepeng/trojan/main/asset/trojan-web.service"
...
installTrojan(){
    ...
    if [[ -z "$lastest_version" ]];then
        colorEcho $red "获取最新版本号失败, 请检查仓库 $version_check 是否已创建 Release 并上传了二进制文件!"
        exit 1
    fi
    echo "正在下载管理程序`colorEcho $blue $lastest_version`版本..."
    [[ $arch == x86_64 ]] && bin="trojan-linux-amd64" || bin="trojan-linux-arm64" 
    curl -fL "$download_url/$lastest_version/$bin" -o /usr/local/bin/trojan
    if [[ $? != 0 ]];then
        colorEcho $red "下载二进制文件失败, 请确保 Release 中存在 $bin 资源!"
        exit 1
    fi
